...
- 双行任务栏歌词与平滑切换动画
- 逐词扫描：当前行按演唱进度逐词高亮
- 多显示器任务栏歌词，可在“显示与外观”中选择全部或指定多台显示器
- 自动识别正在播放的歌曲
- QQ 音乐、网易云音乐、酷狗音乐和 LRCLIB 多来源歌词匹配
- 歌词匹配页：自动匹配选错歌词时，可查看各来源候选与匹配分，人工“本次使用”或“使用并记住”
...
5. 在“显示与外观”页面调整字体、封面、宽度和任务栏位置。
   多显示器用户可在“显示与外观 → 窗口位置 → 显示器”中选择全部显示器或指定一台/多台显示器。
...
TaskbarLyrics 会同时从 QQ 音乐、酷狗、网易云和 LRCLIB 查找歌词，再结合歌曲信息和来源顺序选择最合适的结果。QQ 音乐先找到 90 分以上的结果时，程序会直接使用；否则会等其他来源查找完成后再作选择。如果 QQ 音乐或网易云通过系统媒体信息提供了歌曲 ID，程序会跳过标题搜索，直接尝试获取对应歌词。

### 匹配规则与人工修正

- 程序会比较候选歌词的标题、歌手和时长，匹配分达到 80 分后才可能被采用。QQ 音乐先找到 90 分以上的结果时会直接使用；其他情况下，程序会结合匹配分和来源顺序选择结果。
- 同一标题和歌手的歌曲再次播放时，会直接使用已经保存的歌词，不再重复联网查找。
- 如果自动选择的歌词不正确，可以打开“设置 → 歌词匹配”重新查找。页面会列出各来源的候选歌词、匹配分和未采用原因。选择正确歌词后，“本次使用”只对当前播放生效；“使用并记住”会记住这次选择，以后即使换用其他播放器，遇到相同标题和歌手时也会优先使用。清除歌词缓存会同时删除这些记录。
- 如果不同版本的歌曲拥有完全相同的标题和歌手，例如标题中没有注明 Live 或 Remix，它们会使用同一份已保存歌词。遇到这种情况，可以通过“使用并记住”改成需要的版本。
