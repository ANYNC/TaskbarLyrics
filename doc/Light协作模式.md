...
```md
感谢你提交 TaskbarLyrics Light 版。这个方向很有价值，原生 WPF、不依赖 WebView2 / Chromium，确实能降低进程数、内存占用和发布体积，也给项目提供了一个更轻量的使用形态。

项目最早也是从纯 WPF 实现开始的，后来演进到 WPF + WebView2，是因为我更希望主线优先服务视觉表现和桌面歌词观感。纯 WPF 方案在视觉效果上较难达到我做这个工具的初衷，对 main 来说会更像一次视觉降级。

所以 main 会继续以视觉方向为主，性能优化暂时作为次要考虑，或者等项目更成熟后再做系统性重构。Light 版作为纯 WPF 路线的扩展方向很合适，它可以面向更重视轻量、低占用、少依赖的用户。

因此我不打算把这套实现直接合入 main。它实际上是一条新的 App 层产品线。为了长期维护和发布安全，我会创建 light 分支作为 Light 版的正式开发分支。这个 PR 在修复几个关键问题后，可以考虑合入 light 分支继续维护。

暂时不会开放主仓库直接 push 权限。后续 Light 相关改动可以继续从 fork 向 ANYNC/TaskbarLyrics:light 提 PR；涉及 TaskbarLyrics.Core 的通用修复，建议单独 PR 到 main。

合入 light 前需要先处理：
1. 自动更新源改为 ANYNC/TaskbarLyrics，或先关闭 Light 自动更新。
2. 修复 LyricsWindowHost.Close() 导致窗口关闭和资源清理不执行的问题。
3. 更新安装和发布脚本需要保持可 review、可控。
```
