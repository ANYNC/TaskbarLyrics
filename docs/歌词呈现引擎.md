...
| `layerTransition` | `searchingToSpectrumRoll`、`layerSwitch` | `plan`；`operation: spectrumEntry/spectrumExit`；规范化 `frame`；`isPlaying`、`animateTransition` | 执行检索与频谱跨层翻滚或既有层切换路径 |
...
6. `searching → spectrum` 活动期间的重复频谱目标只覆盖 `pendingFrame`，不能取消 generation、重建 rAF 或把层位置复位到准备态；完成时提交最新频谱目标。
...
1. 稳定频谱收到与当前频谱相同 `trackId` 的临时 `searching` 帧时继续保持频谱，不启动退出；只有不同曲目身份的新检索帧才作为切歌目标进入后续步骤。
...
- 稳定频谱忽略同曲目身份的临时检索帧，切歌时直接翻滚到下一曲目的检索元数据。
