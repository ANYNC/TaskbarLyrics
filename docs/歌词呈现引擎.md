...
| `presentation.js` | 帧规范化、纯转换规划、四种底层转场映射、调度和转场资源所有权 | 直接操作歌词 DOM、读取业务服务 |
| `app.js` | 提供四个底层转场实例并执行歌词、封面和频谱 DOM 更新 | 重新建立第二套场景或计划到实例的映射规则 |
...
| 字段 | 含义 |
| --- | --- |
| `scene` | `searching`、`lyrics`、`spectrum`、`noPlayback` 或 `message` |
...
| `spectrum` | 当前显示目标是频谱层 |
| `noPlayback` | 无播放倒计时和等待播放状态；用于限定自动隐藏的双向翻滚边界 |
| `message` | 未找到歌词、错误和其他普通状态提示 |
...
| `replaceInPlace` | 同一歌词身份的文字修正、布局切换或同一曲目检索内容变化 |
| `singleRoll` | 普通歌词换行、进入检索、检索阶段切歌或普通消息变化 |
| `translationPairRoll` | 带翻译歌词组推进，或检索进入翻译歌词 |
| `searchingToSpectrumRoll` | 检索提示跨层翻滚到频谱 |
| `noPlaybackToSpectrumRoll` | 无播放提示跨层翻滚到频谱 |
...
2. `searching → spectrum` 生成 `searchingToSpectrumRoll`，`noPlayback → spectrum` 生成 `noPlaybackToSpectrumRoll`；其他进入或离开频谱的场景生成 `layerSwitch`。
3. 非检索场景进入 `searching` 使用 `singleRoll`；`searching → searching` 时，不同非空 `trackId` 表示切歌并使用 `singleRoll`，同一曲目的内容校正才做补丁或原地替换。
4. 曲目身份和行索引相同的歌词帧，内容不变时做 `progressPatch`，文字或布局变化时做 `replaceInPlace`。
5. 不同歌词行按目标布局选择 `singleRoll` 或 `translationPairRoll`。
6. `searching/noPlayback → lyrics` 同样按目标布局选择单行或翻译对翻滚；进入 `searching/noPlayback` 也使用 `singleRoll`。
7. 无播放首次进入允许动画，后续倒计时由宿主明确请求立即应用，因此数字在原位更新；普通消息保持原有规划规则。

不要在 `app.js` 的 DOM 执行函数中重新复制这张决策表。新增转换规则应先进入 Planner，并通过纯逻辑测试固定。

### 4.1 四种底层转场

业务计划通过 `TransitionDispatcher` 统一映射到四个底层实例。业务计划描述“为什么转场”，底层实例只描述“如何执行”；两层名称不能互相替代。

| 底层实例 | 业务计划 | 当前调用参数 | 执行职责 |
| --- | --- | --- | --- |
| `patchTransition` | `progressPatch` | `plan`；`operation: lyricsFrame`；规范化 `frame` | 在稳定展示身份内更新进度、逐词进度和次行状态 |
| `replaceTransition` | `replaceInPlace`、`immediate` | `plan`；`operation`；规范化 `frame`；频谱入口额外携带 `isPlaying`、`animateTransition` | 无动画提交歌词帧，或立即进入/退出频谱最终状态 |
| `rollTransition` | `singleRoll`、`translationPairRoll` | `plan`；`operation: lyricsFrame`；规范化 `frame` | 根据目标 `layout` 执行单行或翻译对轨道翻滚 |
| `layerTransition` | `searchingToSpectrumRoll`、`noPlaybackToSpectrumRoll`、`layerSwitch` | `plan`；`operation: spectrumEntry/spectrumExit`；规范化 `frame`；`isPlaying`、`animateTransition` | 执行检索/无播放提示与频谱跨层翻滚或既有层切换路径 |

每个 handler 的统一签名是 `handler(plan, parameters)`。`plan` 保留 `kind`、`from`、`to`、`durationMs`；`parameters` 保留调用场景和完整目标帧。未知计划、缺失实例或不支持的 operation 必须明确失败，不能静默降级到另一种视觉行为。
...
5. 活动转场期间的普通帧只保留最新一帧；首次进入 `noPlayback` 会抢占旧歌词/检索翻滚并重新生成翻滚，倒计时立即更新和频谱场景反转可以中断旧转场。
6. `searching/noPlayback → spectrum` 活动期间的重复频谱目标只覆盖 `pendingFrame`，不能取消 generation、重建 rAF 或把层位置复位到准备态；完成时提交最新频谱目标。
7. 频谱退出期间收到 `noPlayback` 时，立即用无播放提示替换隐藏歌词层目标并继续当前跨层翻滚，不能先展示已排队的旧检索行。

切歌检索不再设置独立的最短停留定时器。新曲目检索行正在执行 `560ms` 翻滚时，快速返回的歌词或频谱只覆盖 Coordinator 的 `pendingFrame`；翻滚完成后立即消费最新目标。这样以真实转场完成事件作为衔接边界，不重复维护一套与动画时长耦合的等待时间。
...
| 检索阶段直接切歌 | `560ms` | 下一曲目的检索元数据按普通单行节奏完整翻入；完成后立即衔接已到达的歌词或频谱 |
| 翻译对翻滚 | `760ms` | 原文和翻译作为一组推进 |
| 检索与频谱互相切换 | `760ms` | 与翻译对相同节奏，使用一个真实 `--line-pitch` 的跨层翻滚 |
| 其他层切换 | `300ms` | 普通歌词或消息与频谱层的兼容切换 |

检索到频谱的执行顺序：

1. 保持检索提示，不写入“纯音乐”或“暂未找到歌词”等中间终态文字。
2. 歌词层位于当前位置；频谱层在禁用 transition 的 `spectrum-transitioning` 准备态以内联样式预置在下方一个 `linePitch` 且透明，预定位本身不产生下移或回弹。
3. 强制读取一次布局，注册转场监听器，并由 Planner 的 `durationMs` 写入统一的跨层转场时长变量。
4. 由 Coordinator 持有的下一帧回调进入 `spectrum-entry-active` 并移除预定位行内样式，让 CSS 终态驱动歌词上移淡出、频谱单向上移淡入。
5. 完成或兜底收尾时清除临时 class 和行内样式，提交 `spectrum` 目标帧。

频谱稳定显示后离开到 `searching`、`lyrics` 或 `message` 时，使用同一 `layerSwitch` 计划但采用相反方向：

1. 稳定歌词或频谱收到与当前内容相同 `trackId` 的临时 `searching` 帧时继续保持稳定内容，不降级为检索；只有不同曲目身份的新检索帧才作为切歌目标进入后续步骤。该规则也保证播放器关闭时可由最后的稳定歌词直接翻入无播放提示，而不经过同曲检索行。
2. 保持 `spectrum-mode`，先通过不提交场景的内容渲染入口将目标帧写入隐藏歌词层，并把歌词层预置在正的 `--line-pitch` 位置。
3. Coordinator 持有的下一帧加入 `spectrum-exit-active`，歌词层向 `0` 上移，频谱层向负的 `--line-pitch` 上移并淡出；目标是 `searching` 时消费 Planner 提供的 `760ms`，其他目标继续使用 `300ms`。
4. 转场期间只保留最新目标帧；频谱反向到达、减少动态效果或显式 `animateTransition = false` 会取消转场并清理 class、rAF、监听器和行内样式。
5. 完成后提交最新目标帧；如果期间已经收到歌词结果，频谱退出完成后立即启动歌词翻滚，不再追加检索停留。
...
原始播放快照在进入可见性状态机、封面更新、`LyricSyncService` 和 WebView 协议之前，先经过宿主侧的 `PlaybackSnapshotStabilityGate`。该边界只暂存“未播放、来源与标题不变、没有不同可靠 `SongId`，且歌手信息仅发生格式变化或信息退化”的弱身份变化；普通切歌、播放中的变化和无播放立即放行。

弱身份候选以 `750ms` 静默期确认，并以 `1500ms` 为最长暂存上限。暂存期间恢复原身份会丢弃候选，进入无播放会立即清空候选并从最后稳定内容进入无播放提示；候选持续存在则在期限到达后交给原有身份、检索和呈现流程。歌手规范化只用于瞬态判定，不得改写稳定 `trackId`、缓存键或歌词偏移身份。门控复用宿主现有轮询推进，不建立独立定时器。
...
文字变化、错误文案调整或倒计时数字变化本身不构成新场景；`noPlayback` 的场景身份来自独立的播放输入状态，不由倒计时文案推断。
...
- 曲目切换进入检索，并保留完整场景与曲目身份；检索阶段直接切歌时翻滚到下一曲目元数据，同曲信息校正仍原位更新。
- 快速歌词或频谱结果不打断检索切歌翻滚，并在该翻滚完成后立即消费最新目标，不产生额外停留。
- 检索到频谱不渲染中间终态文字。
- 检索到频谱启动下一帧后，预定位行内样式已交给 CSS 终态。
- 检索与频谱双向切换使用 `760ms`，普通层切换仍使用 `300ms`，频谱进入准备态不产生预定位动画。
- 活动的检索到频谱转场收到重复频谱帧时不重启，首个启动 rAF 保持有效且完成后提交最新目标。
- 稳定歌词和频谱忽略同曲目身份的临时检索帧，切歌时直接翻滚到下一曲目的检索元数据。
