...
- 全部在线来源并发检索，按来源可信顺序优先选择。
- 首选来源 QQ 音乐取得 90 分以上高可信结果时立即采用，无需等待其他来源。
- 官方播放器提供歌曲 ID 时，对应来源可直接获取歌词，跳过搜索变体。
...
sequenceDiagram
    ...
    participant F as 在线歌词源（QQ 音乐 / 酷狗 / 网易云 / LRCLIB）
    ...
    alt 映射为纯音乐或强制来源
        ...
        alt 本地歌词有效
            ...
            par 按可信顺序并发检索全部在线源
                R->>F: 官方播放器提供 SongId 时对应源直取
                R->>F: 其余来源分段变体搜索
            end
            note over R,C: 首选 QQ 音乐先完成且 ≥90 分时立即录用并取消其余来源
            F-->>R: 多个候选结果
            R->>C: 标题、歌手、时长与来源权重评分
            C-->>R: 可信优先、分数次之的最佳候选
        end
...
4. 在线检索按可信顺序（QQ 音乐 → 酷狗 → 网易云 → LRCLIB）并发启动全部来源；官方播放器（QQ 音乐、网易云、酷狗）上报 `SongId` 时，对应来源跳过搜索变体、直接以 `SongId` 构建候选。
5. 最先等待的首选来源（QQ 音乐）成功取得 `90` 分以上加权结果时，立即取消其余来源并采用；否则等待全部来源完成。
6. 全部来源完成后，若存在任一 `90` 分以上结果，仅在这些高可信结果中选择；否则在全部成功结果中选择。排序为可信顺序优先、身份分数次之。
7. 在线提供者单次最长等待 `5 s`。
8. 使用 `LyricMatcher` 比较标题、歌手和时长，再叠加 `LyricMatchingPolicy` 的来源质量权重；基础匹配分低于 `80` 的在线候选不会进入最终竞争。
...
在线歌词使用两层缓存。来源原文与解析结果按“歌词源＋候选 ID”写入 `%APPDATA%\TaskbarLyrics\database\lyric-pipeline-*-v1.json`，用于避免重复下载和解析；最终采用的歌词写入 `resolved-lyrics-v1.json`，在规则映射、本地歌词和在线检索之前查询。

最终歌词缓存键只由规范化后的标题和歌手构建：字段经过 Unicode FormKC、首尾清理、连续空白折叠和不区分大小写处理后，以版本化 canonical 字符串计算 SHA-256。专辑、时长、播放器、播放器歌曲 ID、歌词源和候选 ID 均不参与 key；这些元数据只保留在缓存 value 中用于诊断。自动检索的最终选择与歌词匹配中的“使用并记住”写入同一种记录，命中后直接应用，不再进行专辑、时长或分数二次校验；“本次使用”不写缓存。
...
Lyricify 在线源与本地歌词解析会过滤歌曲开头 5 秒内可识别的作词、作曲、编曲等制作信息，避免它们抢占首个歌词画面。切歌进入检索时，Web UI 在 `searching` 场景显示两行：第一行由当前 `TrackInfo` 生成“歌曲名 - 歌手”（歌手为空时省略分隔符），第二行保留“正在检索歌词...”。达到 900ms 最短可见时间后，第一帧真实歌词再排入同一滚动队列，而不是直接替换两行文本。状态只保留当前过渡所需的少量帧，不会随切歌次数持续累积。
...
- `Web/Lyrics/bridge.js`：WebView V1 消息入口。
- `Web/Lyrics/state.js`：共享的基础状态规范化函数。
- `Web/Lyrics/presentation.js`：展示帧规范化、转换规划和转场生命周期协调。
- `Web/Lyrics/app.js`：消费转换计划并执行歌词、封面和频谱 DOM 更新。

运行时 `MainWindow.GetLyricsWebUiHtml()` 将 CSS 和 JS 内联到 HTML 后通过 `NavigateToString` 加载。脚本必须按 `bridge.js → state.js → presentation.js → app.js` 的顺序注入，且不能删除 `{{STYLE_CSS}}` 和 `{{APP_JS}}` 占位符。歌词呈现的状态模型、转换规则、生命周期和扩展方式见[歌词呈现引擎](歌词呈现引擎.md)。
...
- 检索双行的生命周期由 payload 的 `scene: "searching"` 驱动，不依赖第一行可见文案；缺少 `scene` 的旧消息仅在展示帧规范化时按历史文案安全推断。快速切换到另一首仍在检索的歌曲时，标题/歌手元数据会立即更新。
- 检索确认进入频谱时，保持检索提示并以一个真实行高向上翻出，频谱层从下一行位置翻入；不会先渲染“纯音乐”或“暂未找到歌词”等中间终态文字，封面位置不参与切换。
...
- 系统字体；默认使用随程序内置的思源黑体。
- 细体、常规、中等、半粗和粗体五档字重。
- 跟随 Windows 任务栏主题、固定深色、固定浅色或自定义文字颜色。
- 文字阴影。

歌曲封面可以配置：

- 基础封面大小，范围为 `12～200 px`。
- 封面与歌词的间距。
- 封面圆角。
- 封面默认显示，可在“尺寸与布局”中关闭；关闭后歌词占满可用宽度，已保存的封面大小、间距和圆角保持不变，重新开启即可恢复。
- 无封面资源时显示与播放器对应的文字和颜色回退图。

整体尺寸可以在 `25%～300%` 之间调整。缩放会根据基础字号、封面、间距和内部布局参数计算实际尺寸，不使用 CSS `scale()`；设置页会显示实际字号、封面和间距。整体缩放、封面显隐和高级尺寸微调收纳在同一“布局与封面”面板中；高级尺寸默认折叠，并可单独恢复推荐比例。频谱在可用宽度不足时会按当前柱数压缩柱宽和间距，并保留每根柱至少一个物理像素，避免高倍率下被挤压消失。

设置页中的标准滑块统一提供紧凑数值输入：滑块用于快速调整，数字框用于精确输入并在有效时即时预览，确认或失焦后持久化。背景不透明度以百分比显示，色相保留专用彩虹轨道但复用同一双输入骨架。
...
外观设置由 C# 统一计算为有效布局尺寸，再传给歌词 Web UI 并写入 CSS 自定义属性。字号等连续渲染值保留小数；封面、间距、内边距、行高和频谱几何等需要像素对齐的值按当前窗口 DPI 换算到物理像素后取最近整数，再换回设备无关单位，窗口高度按物理像素向上取整。窗口宽度和屏幕坐标仍由 WPF 控制；歌词内容排版和动画由 WebView2 控制。
...
窗口高度会结合缩放后的字体、封面和布局间距动态计算；隐藏封面时仅由文字和最低布局高度决定，不保留封面占位。缩放时尽量保持歌词区域垂直中心稳定，接近主屏幕边缘时再移回可见区域。水平锚点决定基础坐标，X/Y 偏移只做相对修正且不随整体倍率缩放。窗口还被标记为工具窗口，避免像普通应用窗口一样出现在任务切换界面中。
...
- “清理缓存”会清理来源原文、解析结果和最终歌词缓存，也会删除歌词匹配中“使用并记住”的指定记录；不会主动清空 `LocalMediaCoverProvider` 的进程内封面缓存，该缓存会随应用重启或相关服务重建释放。
- 最终歌词 key 刻意不包含专辑和时长。同一歌手的同名版本若标题本身没有 Live、Remix 等版本标记，会共用一条最终歌词缓存；可通过歌词匹配重新指定，或清除歌词缓存后重新检索。
