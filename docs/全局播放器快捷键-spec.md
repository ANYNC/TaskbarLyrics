...
| 操作 | 默认快捷键 |
| --- | --- |
| 显示 / 隐藏歌词 | Ctrl+Shift+D |
| 播放 / 暂停 | Ctrl+Shift+P |
| 上一首 | Ctrl+Shift+Left |
| 下一首 | Ctrl+Shift+Right |
...
3. As a user, I want to show or hide the lyrics window with Ctrl+Shift+D by default, so that I can quickly clear or restore the taskbar area.
4. As a user, I want to toggle play and pause with Ctrl+Shift+P by default, so that I can control music without leaving my current application.
5. As a user, I want to go to the previous track with Ctrl+Shift+Left by default, so that I can quickly return to a song.
6. As a user, I want to go to the next track with Ctrl+Shift+Right by default, so that I can skip a song quickly.
