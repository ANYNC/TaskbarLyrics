...
- [x] 删除遗留设置、死方法和无效参数：频谱改为单一模式值；已移除的本地歌词扫描与歌词 WebView 样式方法不再保留死分支。
- [x] 用单一快捷键定义表消除默认键位、动作键、状态键和设置字段的重复映射：设置页列表由 `MediaHotkeyCatalog` 描述数据渲染。
- [x] 统一命名、目录与格式：新增前端模块入口、`.editorconfig` 与集中工程属性，并将现有 C# 格式化为统一基线。
- [x] 清理历史式和误导性注释：删除仅描述过往提交状态的注释；兼容性和播放器限制说明继续保留。
- [x] 启用分析器和格式验证：启用推荐 .NET 分析器，不将存量诊断升级为构建错误；统一验证脚本加入 `dotnet format --verify-no-changes`。
...
- 当前测试为 App 37 项、Core 19 项、前端 4 项，合计 60 项。
